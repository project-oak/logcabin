...
use crate::proto_mapping::IntoProto;
    ...
    fn append_entry(&mut self, request: AppendEntryRequest) -> Result<AppendEntryResponse, Status> {
        ...
        let result = endorser
            ...
            .map_err(|err| match err {
                ...
                AppendEntryError::IndexOverflow => Status::new_with_message(
                    StatusCode::ResourceExhausted,
                    format!("ledger {} has exhausted its index space", request.ledger_id),
                ),
            })?;
...
mod tests {
    ...
    fn append_entry_happy_path() {
        ...
        assert_eq!(resp.append_receipt.len(), 64);
        p256::ecdsa::Signature::from_slice(&resp.append_receipt)
            .expect("append_receipt must be a valid ECDSA P-256 signature");
    ...
    fn read_latest_happy_path() {
        ...
        p256::ecdsa::Signature::from_slice(&resp.entry_receipt)
            .expect("entry_receipt must be a valid ECDSA P-256 signature");
        assert_eq!(resp.read_receipt.len(), 64);
        p256::ecdsa::Signature::from_slice(&resp.read_receipt)
            .expect("read_receipt must be a valid ECDSA P-256 signature");
    }
